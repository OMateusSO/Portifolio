  ...
  <div :class="{ 'dark-theme': lightDark }" class="relative min-h-screen flex flex-col bg-gray-100 dark:bg-gray-900">
    <div class="content w-full max-w-6xl mx-auto p-4 lg:p-8">
      ...
      <div class="py-4 text-center">
        <h1 class="text-3xl lg:text-5xl font-bold dark:text-white">Meus Projetos</h1>
      </div>
      ...
      <div class="cards-container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div 
          ...
          :key="project.id"
          :class="{'bg-white': !lightDark, 'dark:bg-gray-700': lightDark}"
          class="card p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200"
          @click="goToProject(project.route)"
        >
          <h2 class="text-2xl text-black dark:text-white font-semibold mb-2">{{ project.title }}</h2>
      ...
      <!-- Seção de vídeo -->
      <div class="py-8">
        <h1 class="text-3xl lg:text-5xl font-bold text-center dark:text-white">Vídeo de Trabalhos e Projetos</h1>
      </div>
...
const projects = [
  ...
  { id: 3, title: "Biblioteca", description: "Projeto completo de e-commerce com carrinho de compras.", route: "/projetos/biblioteca" },
